...
import { CalendarDays, MapPin, Star, Stethoscope, Users, BriefcaseMedical, ArrowUpRight } from "lucide-react";

function getStatValue(
  stats: { id: string; label: string; value: string }[],
  options: { id?: string; labelPattern?: RegExp; fallback: string }
) {
  const byId = options.id ? stats.find((item) => item.id.toLowerCase() === options.id?.toLowerCase()) : undefined;
  if (byId?.value) return byId.value;

  const byLabel = options.labelPattern ? stats.find((item) => options.labelPattern?.test(item.label)) : undefined;
...
function getStatusClasses(status: string) {
  ...
  if (normalized.includes("online") || normalized.includes("available")) {
...
function getInitials(name: string) {
  ...
  return tokens.slice(0, 2).map((item) => item[0]?.toUpperCase() ?? "").join("");
}
...
export default function DoctorDashboard() {
  ...
  const currentDateLabel = useMemo(
    () => new Intl.DateTimeFormat("en-US", { weekday: "long", month: "short", day: "2-digit", year: "numeric" }).format(new Date()),
    []
  );
  ...
  const dashboardStats = [
    {
      ...
      value: getStatValue(doctor.stats, { id: "patients", labelPattern: /patient/i, fallback: "0" }),
      icon: <Users className="h-4 w-4" />,
    },
    {
      id: "experience",
      label: "Experience",
      value: getStatValue(doctor.stats, { id: "experience", labelPattern: /experience/i, fallback: "0" }),
      icon: <BriefcaseMedical className="h-4 w-4" />,
    },
    {
      id: "rating",
      label: "Rating",
      value: getStatValue(doctor.stats, { id: "rating", labelPattern: /rating/i, fallback: "0" }),
      icon: <Star className="h-4 w-4" />,
    },
    { id: "today", label: "Today Appointments", value: String(todayAppointments.length), icon: <CalendarDays className="h-4 w-4" /> },
  ];
      ...
      <section className="mx-auto max-w-6xl space-y-6">
        <Card className="overflow-hidden border-0 bg-gradient-to-r from-cyan-600 via-sky-600 to-indigo-600 p-0 shadow-[0_24px_45px_-24px_rgba(14,116,144,0.7)]">
          <div className="flex flex-col gap-5 p-5 text-white md:flex-row md:items-end md:justify-between md:p-7">
            <div className="flex items-start gap-4">
              ...
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.2em] text-cyan-50">Doctor Dashboard</p>
                <h1 className="mt-1 text-2xl font-bold md:text-3xl">{doctor.doctorName}</h1>
            ...
            <div className="space-y-2 md:text-right">
              <p className="text-xs font-medium text-cyan-100">{currentDateLabel}</p>
              <span className={`inline-flex rounded-full border px-3 py-1 text-xs font-semibold ${getStatusClasses(doctor.status)}`}>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {dashboardStats.map((stat) => (
            <Card key={stat.id} className="rounded-2xl border border-slate-200/80 bg-white/95 p-4 shadow-[0_12px_28px_-22px_rgba(15,23,42,0.55)]">
              <div className="inline-flex rounded-lg bg-slate-100 p-2 text-slate-600">{stat.icon}</div>
              <p className="mt-3 text-xs uppercase tracking-wide text-slate-500">{stat.label}</p>
